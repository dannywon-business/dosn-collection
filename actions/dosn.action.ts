...
export const getAllProducts = async (authorName: string) => {
  ...
  const allProducts = await fetch(`https://api-asia.mgt.gg/v1${path}?authorName=${authorName}`, {
    method: "GET",
...
export const getBestProducts = async (authorName: string, limit: number) => {
  ...
  const bestProducts = await fetch(`https://api-asia.mgt.gg/v1${path}?authorName=${authorName}&limit=${limit}`, {
    method: "GET",
    headers: { apikey: "PfmoiKctj04qpw93XCTug6bln81HhYrN" },
    next: { revalidate: 0 }
  }).then((res) => res.json());
...
export const scoreProduct = async (prodIdx: number, score: number) => {
  ...
  const updateResult = await fetch(`https://api-asia.mgt.gg/v1${path}`, {
    method: "POST",
    mode: "cors",
